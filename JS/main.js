...
function initTemplate(targetMenu = 'Credit') {
    ...
    if (dynamicArea) {
        ...
        fetch(`Pages/${fileName}.html`).then(r => r.text()).then(html => {
            ...
            else if (fileName === 'caramain') initCaramainUI();
        });
...
// --- LOGIKA TUTORIALS (CARA MAIN) ---
let caramainLang = 'en';

function initCaramainUI() {
    caramainLang = 'en';
}

function toggleCaramainLang() {
    caramainLang = caramainLang === 'en' ? 'id' : 'en';
    const isID = caramainLang === 'id';
    const enDiv = document.getElementById('caramain-en');
    const idDiv = document.getElementById('caramain-id');
    const btn   = document.getElementById('caramain-lang-btn');
    if (enDiv) enDiv.style.display = isID ? 'none' : '';
    if (idDiv) idDiv.style.display = isID ? ''     : 'none';
    if (btn)   btn.textContent     = isID ? '🇺🇸 English' : '🇮🇩 Indonesia';
}
